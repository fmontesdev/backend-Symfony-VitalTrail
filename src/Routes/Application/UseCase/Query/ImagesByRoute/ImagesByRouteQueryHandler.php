...
#[AsMessageHandler]
class ImagesByRouteQueryHandler
{
    ...
    /**
     * Selecciona todas las imagenes de una ruta
     *
     * @param ImagesByRouteQuery $query
     * @return ImageRouteDto[]
     */
    public function __invoke(ImagesByRouteQuery $query): array
    {
        // Verifica si la ruta existe y si el usuario tiene permisos para acceder a ella
        $route = $this->routeRepository->findById($query->idRoute);
        if (!$route) {
        ...
        $images = $this->imageRepository->findImagesByRoute($query->idRoute);
        if (empty($images)) {
            throw new RouteImagesNotFoundException($query->idRoute );
        }
